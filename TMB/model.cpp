...
Type objective_function<Type>::operator() () {
  ...
  if (use_logistic_normal == 1) {
    neg_ll_fishery_age = NLLlogistnorm(fishery_at_age_obs, fishery_at_age_exp_LN_test, fishery_at_age_error_LN_test,exp(log_norm_sigma_fishery),exp(log_norm_phi_fishery),ages,ARMA);
    neg_ll_survey_age = NLLlogistnorm(survey_at_age_obs, survey_age_expectations, survey_at_age_error,exp(log_norm_sigma_survey),exp(log_norm_phi_survey),ages,ARMA);
