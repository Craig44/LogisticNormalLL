...
{0.03833, 0.15238, 0.13923, 0.13306, 0.11866, 0.10056, 0.07909, 0.05446, 0.03615, 0.02724, 0.02345, 0.02156, 0.01914, 0.01549, 0.01181, 0.00881, 0.00645, 0.00460, 0.00951}};
  ...
  // Load a comparison for testing.
  map<unsigned,vector<Comparison>> Compdata;
  Comparison new_comparison;
  unsigned row_iter = 0;
  for(unsigned year = 1990; year < 2014; ++year) {
    unsigned age = 1;
    for (auto i = 0; i < exp_mat.size();++i) {
      new_comparison.age_ = age;
      new_comparison.observed_ = obs_mat[row_iter][i];
      new_comparison.expected_ = exp_mat[row_iter][i];
      ++age;
      Compdata[year].push_back(new_comparison);
    }
    ++row_iter;
  }
 ...
 */ /*
   * Testing the time taken to inverse a matrix with my code vs boost, if it is a huge difference we may have to persist with boost somehow.
   *
  */
  bool sexed ,ARMA = false;
  unsigned N_ages = 30;
  double sigma = 0.3;
  vector<double> phi = {0.3,0.2};
  vector<vector<double>> covar = covmat_logistnorm(sigma, phi, N_ages,false, false, ARMA, sexed);
  boost::numeric::ublas::matrix<double> cov_boost(N_ages,N_ages,0.0);
  boost::numeric::ublas::matrix<double> cov_boost_inv(N_ages,N_ages,0.0);
  for(unsigned i = 0; i < covar.size(); ++i) {
    for(unsigned j = 0; j < covar[i].size(); ++j) {
      cov_boost(i,j) = covar[i][j];
    }
  }
  clock_t start;
  start = clock();
  bool inverted = InvertMatrix(cov_boost,cov_boost_inv);
  if (inverted)
    cout << "inverted matix and took = " << clock() - start << " milliseconds" << endl;
  else
...
void rlogistnorm(map<unsigned,vector<Comparison>>& comparisons, unsigned N, double sigma, vector<double> phi, vector<vector<double>> covmat, bool ARMA, bool sexed){
   map<unsigned,double> year_totals;
   ...
   for (auto year_iterator = comparisons.begin(); year_iterator != comparisons.end(); ++year_iterator) {
     ...
     for (Comparison& comparison : year_iterator->second) {
       ...
       year_totals[year_iterator->first] += comparison.observed_;
       ++vec_iter;
     }
   }
   ...
   for (auto year_iterator = comparisons.begin(); year_iterator != comparisons.end(); ++year_iterator) {
     unsigned vec_iter = 0;
     for (Comparison& comparison : year_iterator->second) {
       comparison.observed_ /= year_totals[year_iterator->first];
       ++vec_iter;
     }
   }
